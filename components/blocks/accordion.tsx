...
const AccordionItem = ({ data, index, cardstyle, isLast, parentField = "" }) => {
  ...
  const borderWidthClass = `border-b-${borderWidth(cardstyle?.borderStyles)}`
  ...
  return (
    <div className={`${borderWidthClass} first:border-t-0 ${borderColor(cardstyle?.borderStyles)} ${cardstyle?.fillStyles}`}>
      <div className={`relative cursor-pointer text-primary pr-10 ${cardstyle?.padding} ${cardstyle?.headlineStyles} ${active && cardstyle?.fillStylesActive }`} onClick={() => setActive(!active)}>
        ...
        <div className={`absolute z-20 right-5 top-6 sm:top-4 ${active ? 'transform rotate-180' : ''}`}>
      ...
      <div className={`text-body1 lg:text-body-lg  ${borderColor(cardstyle?.borderStyles)} ${active ? '' : 'hidden'}`}>
        <div
          className={`relative flex-1 border-box pb-4`}
        >
